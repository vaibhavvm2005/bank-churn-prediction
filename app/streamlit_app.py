...
model, scaler, features, feature_importance = load_models()
if model is None:
    st.stop()
...
col2.metric("Risk Level", "High" if churn_prob > 0.5 else "Medium" if churn_prob > 0.3 else "Low")
col3.metric("Recommendation", "Immediate action" if churn_prob > 0.5 else "Monitor" if churn_prob > 0.3 else "Normal")

fig = go.Figure(go.Indicator(mode="gauge+number", value=churn_prob*100, title={"text": "Risk Score"}, gauge={"axis": {"range": [0,100]}, "steps": [{"range": [0,30], "color": "green"}, {"range": [30,70], "color": "yellow"}, {"range": [70,100], "color": "red"}]}))
